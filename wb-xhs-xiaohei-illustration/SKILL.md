...
在生成前先写 `art_direction`：当前文章的矛盾或变化、一个新造的物理动作与低科技物件、主体的取景角度、留白位置和批注节奏。高质量不是把“办公桌、电脑、便签”换一种摆法；画面必须让读者不看正文也能感到这篇文章在讨论的具体关系。

`quality_gate` 按**叙事动作、线稿完成度、留白、批注和原创性**逐项审查：

- `叙事动作`：小黑（或明确选择 `pencil_narrative` 的非特定人物）真的推动关系变化，不能只站在道具旁。
- `线稿完成度`：动作、物件比例和空间关系清楚；线条有前后层次，不出现肢体、透视、物件连接或文字的明显错误。
- `留白`：主体约占规定比例，画面在手机尺寸仍呼吸；铅笔变体保留顶部标题位。
- `批注`：只保留能帮助理解的短中文，颜色职责明确，不把术语或乱码当装饰。
- `原创性`：隐喻、动作和物件来自当前文章，既不复刻来源示例，也不回退为无关的通用办公场景。

先生成候选，再记录 `quality_gate` 结果。若失败，只修失败的动作、线稿、留白、批注或隐喻；不能因为满足白底和画幅就把平庸候选标为完成。
