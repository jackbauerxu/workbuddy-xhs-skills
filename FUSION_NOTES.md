...
本次新增来源：[yuwen-cool/yuwen-publish-precheck](https://github.com/yuwen-cool/yuwen-publish-precheck)。来源仓库采用 CC BY-NC 4.0；本项目只吸收其“发布前审不是关键词命中题，而是候选扫描 + 上下文语义判断 + 修复后复检”的方法，并将其落到 `wb-xhs-humanize-compliance`，没有复制规则库、平台文本或绕审提示词。

### 融合进了什么

- **平台身份判断**：先区分小红书目标、商业内容、高风险行业和视觉资产是否属于发布内容的一部分。
- **两层预审**：词面候选扫描只负责找入口，语义复核才判断主张、意图、证据和上下文；零命中也不作为安全证明。
- **风险覆盖**：站外导流、虚假身份、夸张收益/效果、未核验数据与案例、未授权截图/图片、版权、商业披露、标题—封面—正文不一致。
- **保意修复**：披露/声明 → 补证据 → 收窄断言 → 最小改写 → 删除或暂缓；改完重新检查全文、图片文字和行动引导。
- **输出合同**：`可发 / 需修改 / 暂不建议发`，并列出必须改、建议改、待确认、替换文本和复检结果。

### 没有融合什么

- 没有把 `yuwen-publish-precheck` 的规则文件整包复制进本仓库。
- 没有把来源仓库对抖音、视频号的具体规则冒充成小红书官方规则。
- 没有提供谐音、拼音、拆字、表情、图片藏字等规避审核方法。

这让 `wb-xhs-humanize-compliance` 同时承担“改得像真人”和“发布前把事实、证据、导向、商业关系、视觉文字一起过一遍”的职责；需要当前平台细则时仍要以官方规则为准。
