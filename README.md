...
![WorkBuddy XHS Skills：从定位到发布复盘的内容系统](assets/readme/hero.svg)
...
| 入口 | 解决的问题 | 主要交付 |
| --- | --- | --- |
| 定位与变现 | 我为谁提供什么真实价值 | 账号承诺、信任路径、边界 |
| 选题与标题 | 下一条内容为什么值得发 | 选题卡、标题、封面钩子 |
| 初稿与合规 | 怎样像真人且不越界 | 可审查改稿、披露和复检 |
| 视觉与排期 | 怎样把内容变成能交付的图文 | 封面、插图、素材、发布节奏 |
| 数据与复盘 | 下一轮该改哪一条规则 | 证据化结论与账号记忆 |

视觉资产只是表达层，不替代内容证据；没有真实图像工具或授权素材时，流程会保留 `not_called` / `missing_asset` 状态。
...
每张图还要先有内容专属艺术方向，再经过专家质量门：封面检查缩略图、文字、锚点与信任；小黑图检查叙事动作、线稿、留白、批注与原创性；材质图检查结构、标签、材质完成度与层级。首张生成只是候选，只有失败维度修正并通过检查后才标为交付。
...
建议从最小交付开始，不要在缺少标题、来源、授权素材或关键事实时直接生成整套图片。每个视觉 Skill 都要先给出预检、候选方向、QA 结果和真实文件状态。
...
- `yuwen-cool/yuwen-publish-precheck` 的两层预审、平台身份判断、风险分级、保意修复和改后复检方法。

核心变化：去 AI 味不只是口语化，而是让一篇内容同时更像真人、更容易被看完，也能被清楚地拆成图文页；发布前审不把词面命中当最终违规，也不把零命中当安全证明。

### 5.1 2026-08-01 发布前审融合边界

这次融合只把 `yuwen-cool/yuwen-publish-precheck` 的方法转成小红书技能步骤，保留原仓库的署名与 CC BY-NC 4.0 来源说明，不复制其规则库或大段文本。小红书技能会检查：

- 平台身份、商业属性和高风险行业；
- 标题、封面文字、图片文字与正文承诺是否一致；
- 站外导向、虚假身份、未核验案例/数据、版权与授权；
- 候选扫描 → 语义复核 → 保意修复 → 改后复检；
- 对谐音、拼音、拆字、表情、图片藏字等绕审请求明确拒绝，并提供平台内替代动作。

它不会承诺“绝对安全”或“必然不被限流”；需要当前官方规则时，仍须单独核验平台最新公告。
